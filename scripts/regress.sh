...
step "4/8 go test -race ./... (unit + dispatch)"
go test -race -timeout 120s ./... || fail "race tests"
ok "race tests green"

step "5/8 go test -tags=e2e ./... (TUI e2e)"
...
if grep -rl "//go:build integration" --include="*.go" . >/dev/null 2>&1; then
  step "6a/8 go test -tags=integration ./..."
  go test -tags=integration -timeout 120s ./... || fail "integration tests"
  ok "integration tests green"
fi

step "6/8 go test -bench=. -benchmem -run=^$ ./... (benches)"
go test -bench=. -benchmem -run='^$' -timeout 600s ./... || fail "benchmarks"
ok "benchmarks within budget"

step "7/8 doc-sweep (`docs/CONVENTIONS.md` §12.6 mechanical checks)"
bash "$(dirname "$0")/doc-sweep.sh" || fail "doc-sweep"
ok "doc-sweep clean"

step "8/8 lint-agents-md (slim-file budget + symlink + §-anchor cites)"
bash "$(dirname "$0")/lint-agents-md.sh" || fail "lint-agents-md"
ok "agents-md drift clean"
