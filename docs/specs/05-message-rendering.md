...
**Status:** Shipped (CI scope, v0.2.x → v0.17.x → v0.54.0). Headers + body fetch + html2text + plain-text normalisation + URL extraction + OSC 8 hyperlink wrapping all wired. v0.17.x adds the attachment visibility block between headers and body (PR 8 sibling — spec §8 amended). URL extraction hardened against parens-in-query + hard-wrapped URLs in the same release. v0.54.0 lands §6.1.1 — the data-vs-layout `<table>` classifier — wired through `[rendering].pretty_tables` (default `true`); real data tables surface as ASCII grids while MJML layout chrome flattens. Residual: viewer keybindings (`o`, `O`, `e`, `Q`, `1-9`, `a-z`, `Shift+A-Z`, `[`, `]`) + GetAttachment helper + save / open path + thread map + format=flowed unwrapping + attribution-line detection — all tracked under PR 10.
