...
- **TOML on disk** — `~/.config/inkwell/rules.toml` is the
  ...
  to start on any error (CLAUDE.md §9).
...
4. Greps `docs/specs/32-server-side-rules.md` for **both** the
   padded slot literal (`014`) and the unpadded SQL literal
   (`'14'`) and replaces every occurrence (this section, §13
   DoD, tests). Use
   `grep -nE "014|'14'" docs/specs/32-server-side-rules.md`
   as the checklist.
...
// (idempotent — matches CLAUDE.md §3 invariant 3 for mutations).
...
// Duplicated minimally here because per CLAUDE.md §2 layering
...
- `permanentDelete` (irreversible; CLAUDE.md §7 rule 9
...
modal display triggers an immediate re-layout per CLAUDE.md §4.
...
CLAUDE.md §5.2 ("Synthesised fixtures … are generated by helpers
...
`<email-N>` keying (CLAUDE.md §7 rule 3). New log sites MUST go
...
- `display_name` of a rule — may contain a user-typed name like
  ...
  a subject line per CLAUDE.md §7 rule 3 — logged verbatim at
...
- [ ] Migration `014_message_rules.sql` lands cleanly with:
      ...
      re-confirmation of the slot number per CLAUDE.md §13.
...
- [ ] `MessageRulesModel` (value-typed, per CLAUDE.md §4): holds the
...
- [ ] `internal/config`: new `RulesConfig` struct embedded in the
      ...
      rejection.
- [ ] `docs/plans/spec-32.md` exists with `Status: done` at ship
      time, per CLAUDE.md §13's mandatory-plan-file rule (the
...
## 14. Cross-cutting checklist (CLAUDE.md §11)
...
- [ ] **Spec 28 consistency:** screener gates new senders into a
      ...
      folder ID — see `docs/specs/28-screener.md` §4.4 and
