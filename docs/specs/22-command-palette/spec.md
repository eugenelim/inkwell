...
sub-model convention from `docs/CONVENTIONS.md` §4.
...
exceeds 22ms (the 50% headroom rule from `docs/CONVENTIONS.md` §6).
...
- [ ] **Tests** (§9 spec layer):
  ...
  - [ ] e2e (`docs/CONVENTIONS.md` §5.4 — visible-delta required for every binding):
  ...
  - [ ] Bench: `BenchmarkPaletteFilter` over 5000 rows under 22ms
        ...
        binary blobs per `docs/CONVENTIONS.md` §5.2).
...
- [ ] **Project docs** (must land in the same commit that ships the
  ...
  - [ ] `docs/specs/22-command-palette/plan.md` exists and is updated each iteration
        (`docs/CONVENTIONS.md` §13 mandatory tracking note).
...
- [ ] **Spec 20 consistency:** thread-chord verbs (`T r`, `T R`, `T f`,
      ...
      code is in scope** (`docs/CONVENTIONS.md` §12.4 — "Let me refactor while
