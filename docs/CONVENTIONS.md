# CONVENTIONS.md — Inkwell repo conventions

The long-form rules for the repo. The slim entry point is
[`AGENTS.md`](../AGENTS.md) at the repo root (which is also exposed as
`CLAUDE.md` via a symlink for Claude Code, and read by Cursor / Codex /
Copilot / Gemini CLI directly).

**Section numbers (`§N`) are stable contract surfaces** — referenced from
code comments, ADRs, spec/plan files, and per-package `AGENTS.md`s. Don't
renumber casually. Adding a new §N at the end is fine; reordering or
collapsing existing ones requires updating every cite (a few hundred sites
across `docs/`, `internal/`, `cmd/`).

This file is **living**: when reality changes, update the §N that's now
wrong, in the same PR.

> **Landed here directly?** See [`AGENTS.md`](../AGENTS.md) for the canonical
> read order (PRD → ARCH → CONFIG → this file → spec → per-package
> `AGENTS.md` → ADR) and the source-of-truth table.
...
> **Canonical guide:** `docs/TESTING.md`. §5 here is policy;
...
```
inkwell/
├── AGENTS.md                  # slim entry point (CLAUDE.md → symlink)
├── CLAUDE.md                  # → AGENTS.md
├── docs/
...
Moved to [`AGENTS.md` § Quick reference](../AGENTS.md#quick-reference--common-commands).
The slim entry point owns the commands; this section number is preserved so
historical `§15` cites still find a landing anchor.
