...
func runRoot(cmd *cobra.Command, rc *rootContext) error {
	...
	model, err := ui.New(ui.Deps{
		...
		ScreenerMaxCountPerSender: cfg.Screener.MaxCountPerSender,
		ScreenerHintDismissed:     cfg.UI.ScreenerHintDismissed,
		ScreenerLastSeenEnabled:   cfg.UI.ScreenerLastSeenEnabled,
		InboxSplit:                cfg.Inbox.Split,
		InboxSplitDefaultSegment:  cfg.Inbox.SplitDefaultSegment,
		InboxSplitShowZeroCount:   cfg.Inbox.SplitShowZeroCount,
...
func bindingsToOverrides(b config.BindingsConfig) ui.BindingOverrides {
	return ui.BindingOverrides{
		...
		ScreenerAccept:   b.ScreenerAccept,
		ScreenerReject:   b.ScreenerReject,
	}
}
...
func (a *threadExecutorAdapter) ThreadExecute(ctx context.Context, accID int64, verb store.ActionType, focusedMsgID string, params map[string]any) (int, []ui.BulkResult, error) {
	total, results, err := a.exec.ThreadExecute(ctx, accID, verb, focusedMsgID, params)
