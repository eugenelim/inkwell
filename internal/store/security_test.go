...
// system cannot read mail content. `docs/CONVENTIONS.md` §7 rule 1 promises this;
...
func TestDatabaseFileMode(t *testing.T) {
	...
	require.Equal(t, os.FileMode(0o600), mode,
		"mail.db must be created with mode 0600 to prevent other users on the same system from reading mail content (`docs/CONVENTIONS.md` §7 rule 1)")
}
